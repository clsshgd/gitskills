...
struct switchtec_ntb {
	...
	dma_addr_t self_shared_dma;
	 ...
	 void *self_bar4_shared;//self share memory vaddr
	 void  *__iomem *peer_bar4_shared;//ntb xlink share mem vaddr, this hit local ep2 bar2 lut0 mem addr then deliver to remote bar2 lut0-> remote address
	dma_addr_t self_bar4_shared_dma;
				     ...
				     enum switchtec_msg msg)
{
	...
	if (msg == MSG_LINK_FORCE_DOWN)
        ...
        //memcpy(&i,sndev->self_bar4_shared,sizeof(i));
        printk("data is 0x%x\n",ioread32(sndev->self_bar4_shared));
        printk("spad15 is 0x%x\n",ioread32(&sndev->self_shared->spad[15]));
        mb();
      //  printk("data is 0x%x\n",i);
			       ...
			       int lut_idx, int partition, u64 addr)
{
	...
	iowrite32(ctl_val, &ctl->bar_entry[peer_bar].ctl);
        //printk("lut partition is %d\n",partition);
	iowrite64((NTB_CTRL_LUT_EN | (partition << 1) | addr),
		  &ctl->lut_entry[lut_idx]);
        //printk("lut entry value is 0x%llx\n",ioread64(&ctl->lut_entry[lut_idx]));
	rc = switchtec_ntb_part_op(sndev, ctl, NTB_CTRL_PART_OP_CFG,
				   NTB_CTRL_PART_STATUS_NORMAL);
	if (rc) {
			       ...
			       u64 *mw_addrs, int mw_count)
{
	...
	for (i = 0; i < sndev->nr_direct_mw; i++) {
		...
		if (offset && size > offset)
			size = offset;
              //  printk("addr is 0x%llx,size is 0x%llx,xlate_pos is %d\n",addr,size,xlate_pos);
		ctl_val = ioread32(&ctl->bar_entry[bar].ctl);
		ctl_val |= NTB_CTRL_BAR_DIR_WIN_EN;
                
		iowrite32(ctl_val, &ctl->bar_entry[bar].ctl);
              //  printk("xlink ctl is 0x%x\n",ioread32(&ctl->bar_entry[bar].ctl));
		iowrite32(xlate_pos | (lower_32_bits(size) & 0xFFFFF000),
			  &ctl->bar_entry[bar].win_size);
               // printk("xlink winsize is 0x%x\n",ioread32(&ctl->bar_entry[bar].win_size));
		iowrite32(upper_32_bits(size), &ctl->bar_ext_entry[bar].win_size);
              //  printk("xlink ext bar winsize is 0x%x\n",ioread32(&ctl->bar_ext_entry[bar].win_size));
		iowrite64(sndev->peer_partition | addr,
			  &ctl->bar_entry[bar].xlate_addr);
              //  printk("xlink xlate is 0x%llx\n",ioread64(&ctl->bar_entry[bar].xlate_addr));
	}
				    ...
				    u64 *bar_addrs)
{
	...
	//	dev_dbg(&sndev->stdev->dev,
		printk(	"Crosslink BAR%d addr: %llx\n",
			i*2, bar_addr);
...
static int switchtec_ntb_init_crosslink(struct switchtec_ntb *sndev)
{
	...
	if (rc)
		return rc;
        printk("barcnt is %d, baraddr[1] is 0x%llx, baraddr[2] is 0x%llx\n",bar_cnt,bar_addrs[1],bar_addrs[2]);
...
//	dev_dbg(&sndev->stdev->dev,
printk( "Peer MWs: %d direct, %d lut\n",
...
#define DW_1MB (1024*1024)

static void switchtec_ntb_init_bar4_shared(struct switchtec_ntb *sndev)
{
	//int i;

	memset(sndev->self_bar4_shared, 0xaa, DW_1MB);
        //memset(sndev->self_bar4_shared,0x55,4);
	//printk("0x%x\n",*(char *)sndev->self_bar4_shared);
	printk("bar4 shared addr:0x%llx\n", sndev->self_bar4_shared);
	printk("bar4 shared dma addr:0x%llx\n", sndev->self_bar4_shared_dma);
//	sndev->self_shared->magic = SWITCHTEC_NTB_MAGIC;
//	sndev->self_shared->partition_id = sndev->stdev->partition;
/*
	for (i = 0; i < sndev->nr_direct_mw; i++) {
		...
		if (i == 0)
			sz = min_t(resource_size_t, sz,
				   LUT_SIZE * sndev->nr_lut_mw);// 2MB 64KB*32 bar 2 is 2MB

		sndev->self_shared->mw_sizes[i] = sz;//bar 4 is still 256G  //0,1 two direct window
	}

	for (i = 0; i < sndev->nr_lut_mw; i++) {
		int idx = sndev->nr_direct_mw + i;

		sndev->self_shared->mw_sizes[idx] = LUT_SIZE;// 2-33 total 32 luts
	}
	*/
}
...
//do nep 0 lut map to ep2 bar 0 peer dmesg register memory align windwo, notice this bar0 will be deliver message to xlink remote bar0
//second time, set ep2 bar 2,
static int config_rsvd_dw_win(struct switchtec_ntb *sndev,
			       struct ntb_ctrl_regs __iomem *ctl,
			       int lut_idx, int partition, u64 addr)//ctl self ctrl,lut idx 1, partition =peer partition, addr peer db address in nep2
			       										//second call ctl is ep2 ctl, lutidx is 0, partition is 0 partition 0,addr is cpu memory phyical address of shared mem
{
	int peer_bar = sndev->peer_direct_mw_to_bar[1];//always bar4
	u32 ctl_val;
	int rc;
	int xlate_pos;
	size_t size;
    printk("peer bar is %d\n",peer_bar);//should be 4
	rc = switchtec_ntb_part_op(sndev, ctl, NTB_CTRL_PART_OP_LOCK,//lock self  make sensable because change itself
				   NTB_CTRL_PART_STATUS_LOCKED);
	if (rc)
		return rc;
	...
	//bar = sndev->direct_mw_to_bar[i];
		//	offset = (1 == 0) ? LUT_SIZE * sndev->nr_lut_mw : 0;
			//addr = mw_addrs[i] + offset;
		//	size = pci_resource_len(sndev->ntb.pdev, peer_bar);// - offset;
         size=DW_1MB;		
	xlate_pos = ilog2(size);
			...
			ctl_val = ioread32(&ctl->bar_entry[peer_bar].ctl);
                        ctl_val &= 0xFF;
			ctl_val |= NTB_CTRL_BAR_DIR_WIN_EN;
			//we can see bar 2 is 4000200000 size 3fffe00000, bar4 is 80000000 size 4000000000
			iowrite32(ctl_val, &ctl->bar_entry[peer_bar].ctl);
                        printk("ctl is 0x%x\n",ioread32(&ctl->bar_entry[peer_bar].ctl));
			//iowrite32(xlate_pos | size, &ctl->bar_entry[peer_bar].win_size);
			iowrite32(xlate_pos | (lower_32_bits(size) & 0xFFFFF000),
		      &ctl->bar_entry[peer_bar].win_size);
                       printk("win_size is 0x%x\n",ioread32(&ctl->bar_entry[peer_bar].win_size));
	        iowrite32(upper_32_bits(size), &ctl->bar_ext_entry[peer_bar].win_size);
               // printk("ext winsize is 0x%x\n",ioread32(&ctl->bar_ext_entry[peer_bar].win_size));
			iowrite64(partition | addr,
				  &ctl->bar_entry[peer_bar].xlate_addr);
                 //printk("xlate is 0x%llx\n",ioread64(&ctl->bar_entry[peer_bar].xlate_addr));
...
/*
*switchtec_ntb_init_bar4_shared_mw
* Create 1024*1024 B memory share window between NTB
*
*/

static int switchtec_ntb_init_bar4_shared_mw(struct switchtec_ntb *sndev)
{
	int self_bar = sndev->direct_mw_to_bar[1];//self bar will be bar4
	int rc;

	//sndev->nr_rsvd_luts++;//one used xlink dbll,now add another, we just use dw not lut
	sndev->self_bar4_shared = dma_zalloc_coherent(&sndev->stdev->pdev->dev,
						 DW_1MB,
						 &sndev->self_bar4_shared_dma,
						 GFP_KERNEL);//allocate memory 64MB for dma, selfshared is CPU vm address, and self_shared_dma is dma physical address GFP_KERNEL conitnuous
	if (!sndev->self_bar4_shared) {
		dev_err(&sndev->stdev->dev,
			"unable to allocate memory for shared bar4 mw\n");
		return -ENOMEM;
	}

 //          sndev->self_bar4_shared = __get_free_pages(GFP_KERNEL, 10);//2^10 4KB page=4MB
 //       if(sndev->self_bar4_shared)
  //          sndev->self_bar4_shared_dma = virt_to_phys((unsigned long*)sndev->self_bar4_shared);



	switchtec_ntb_init_bar4_shared(sndev);//this memory record magic number as a sector header, descript mem window for lut and dw, but dw[0]-bar 2 size be low to 32lut *64KB size why?

	rc = config_rsvd_dw_win(sndev, sndev->mmio_peer_ctrl, 0,
				 sndev->self_partition,
		           		 sndev->self_bar4_shared_dma);//just set ep2 lut 0 let share memory be target translated mem of ep2 bar 2, other ep2 window not tranlated
	if (rc)
		goto unalloc_and_exit;

	sndev->peer_bar4_shared = pci_iomap(sndev->stdev->pdev, self_bar, DW_1MB);//ep 0 bar 2 io map to cpu memory space size lut64KB//cpu ep0 bar 2 lut 0 -> remote bar2 lut0 address ->remote ep0 share mem
	if (!sndev->peer_bar4_shared) {
		rc = -ENOMEM;
		goto unalloc_and_exit;
	}
        
//	dev_dbg(&sndev->stdev->dev,
     //  printk( "Shared Bar4 MW Ready\n");
	return 0;

unalloc_and_exit:
	dma_free_coherent(&sndev->stdev->pdev->dev, DW_1MB,
			  sndev->self_bar4_shared, sndev->self_bar4_shared_dma);

	return rc;
}

static void switchtec_ntb_deinit_bar4_shared_mw(struct switchtec_ntb *sndev)
{
	if (sndev->peer_bar4_shared)
		pci_iounmap(sndev->stdev->pdev, sndev->peer_bar4_shared);
...
static int switchtec_ntb_reinit_peer(struct switchtec_ntb *sndev)
{
	...
	rc = config_rsvd_lut_win(sndev, sndev->mmio_peer_ctrl, 0,
				 sndev->self_partition,
				 sndev->self_shared_dma);
	//maybe need add config_rsvd_dw_win
	return rc;
}
			     ...
			     struct class_interface *class_intf)
{
 ...
 int j = 0;
    ...
    rc = switchtec_ntb_init_bar4_shared_mw(sndev);//create local share memory and hulk vaddr to remote share mem //risk is kernel is very hard to allocate so much memory even not in module
	if (rc)
		goto deinit_shared_and_exit;
	
	rc = switchtec_ntb_init_db_msg_irq(sndev);
	if (rc)
		goto deinit_shared_and_exit1;
...
#if 0      
  while(i<230){
          printk("spd 15 data is 0x%x\n",sndev->self_shared->spad[15]);
          printk("data 2 is 0x%x\n", *(int*)(sndev->self_bar4_shared));
         // ssleep(1);
        //  j = 0x88aa1234;
          mb();
             //   memcpy(sndev->peer_bar4_shared,&j,sizeof(j));
                mb();
             j=0;
	//	memcpy(&j,sndev->self_bar4_shared,sizeof(j));
                mb();
        //  printk("j is 0x%x\n",j);
         ssleep(1);
          i++;
...
deinit_shared_and_exit1:
	switchtec_ntb_deinit_bar4_shared_mw(sndev);
